<?php
/**
 * @package Internship_Certificate
 * @version 2.0
 */
/**
 * Plugin Name: Internship Certificate Verification
 * Plugin URI: https://awareearth.org/index.php/technology/internship-certificate-verification-wordpress-plugin/
 * Description: Admin can enter certificate Number, and details in the panel and user can verify their certificate using the certificate Number code in the front end.
 * Version: 1.0
 * Author: Avanish Jaiswal
 * Author URI: https://github.com/avanishjaiswal/
 */
...
function Internship_certificate_plugin_styles_scripts() {
...
add_action('admin_enqueue_scripts', 'Internship_certificate_plugin_styles_scripts');

function Internship_certificate_include_bs_datatables() {
...
if( isset($_GET['page']) && $_GET['page'] == 'certificate-codes' ){
	add_action('admin_enqueue_scripts', 'Internship_certificate_include_bs_datatables');
}


function Internship_certificate_include_bootsrap(){ ?>
	<style type="text/css">
		.cf-search {
		    width: 700px;
...
<?php }
add_action('wp_head', 'Internship_certificate_include_bootsrap');
...
register_activation_hook( __FILE__, 'Internship_certificate_avanish_certificate_onActivation' );
register_deactivation_hook( __FILE__, 'Internship_certificate_avanish_certificate_onDeactivation' );

// Search certificate
function Internship_certificate_certificate_search_form(){ 
	$output = '';
	...
	if( isset($_POST['code_data']) ){
		...
		global $wpdb;
		$rows = $wpdb->get_results( "SELECT * FROM avanish_Internship_certificates where certificate_code = '$code'"); 
		if( !empty($rows) ){
	...
	<div class="hscroll">
        <table class="search-table" style="overflow-x:auto;">
        	<thead>
                <tr>
                    <th class="btlr-10">Employee Name</th>
                    <th>Employee Role</th>
                    <th>Duration</th>
                    <th>Certification No</th>
                    <th>Date of Birth</th>
                    <th class="br-0 btrr-10">Certificate Issue Date</th>
                </tr>
            </thead>
            <tbody>';
...
add_shortcode( 'get_Internship_certificate_search_form' , 'Internship_certificate_certificate_search_form' );