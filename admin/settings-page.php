...
function Internship_certificate_admin_certificate_ui() {
		...
		if(!wp_verify_nonce($_POST['course_nonce'], 'admin_certificate_ui')) {
			...
			if (strpos($editid, ',') !== false) {
				...
				foreach( $editid as $edt ) {
					$result = Internship_certificate_delete_Internship_certificate( $edt );
				}
			} else {
				$result = Internship_certificate_delete_Internship_certificate($editid);
			}
			if( $result == 1 ) {
			...
			$editid = sanitize_text_field($_POST['editid']);
			$result = Internship_certificate_add_Internship_certificate($code, $name, $course, $hours, $dob, $award_date, $editid);
			if( $result == 1 ) {
				if( $editid != "" ) {
	...
	global $wpdb;
    $certificates = $wpdb->get_results( "SELECT * FROM avanish_Internship_certificates");
	...
	<style>
		@font-face {
		    ...
		    src: url("../wp-content/plugins/Internship-certificate-verification/assets/css/fonts/VarelaRound-Regular.otf") format("opentype");
		}
	    body {
	        color: #566787;
			background: #f5f5f5;
	...
	<div class="container">
	  <div class="table-wrapper">
	    <div class="table-title">
	      <div class="row">
	        <div class="col-sm-6">
	          <h2 style="color: white;">Manage <b>Internship/Job Certificates</b></h2>
	        </div>
	        <div class="col-sm-6">
	          <a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Add New Certificate</span></a>
	          <a href="javascript:void(0);" class="btn btn-danger deleteMultiple" data-toggle="modal"><i class="material-icons">&#xE15C;</i> <span>Delete certificate</span></a>
	        </div>
	      </div>
	    </div>
	    <?php echo $error;?>
		<div class="alert alert-info" role="alert">
		  	<strong>[get_Internship_certificate_search_form]</strong> Copy and paste the shortcode on the page you want the search bar and result of the certificates to be.
		</div>
